...
// Section 2.6 of the spec. The content is already conformant with the spec, no
// need to change. Only call them at correct times.
...
// Building a packet by filling the header and contents. This function is
// provided to you and you can use it directly
...
int main (int argc, char *argv[])
{
    ...
    //       project. Optionally, you could also consider adding a timeout to
    //       the socket using setsockopt with SO_RCVTIMEO instead.
    ...
    for (int i = 1; ; i++) {
        ...
        // Implement GBN for basic requirement or Selective Repeat to receive
        // bonus

        // Note: the following code is not the complete logic. It only expects a
        //       single data packet, and then tears down the connection without
        //       handling data loss. Only for demo purpose. DO NOT USE IT in
        ...
        while(1) {
            ...
            if (n > 0) {
                ...
                else { //normal packet exchange (non-fin)
                    if (recvpkt.seqnum - cliSeqNum > PAYLOAD_SIZE * (WND_SIZE + 1)) { //sequence number exceeds max and loops around
                        buildPkt(&ackpkt, recvpkt.acknum, (recvpkt.seqnum + recvpkt.length) % MAX_SEQN, 0, 0, 1, 0, 0, NULL);
                        printSend(&ackpkt, 0);
                        sendto(sockfd, &ackpkt, PKT_SIZE, 0, (struct sockaddr*) &cliaddr, cliaddrlen);
                    } 
                    else if (recvpkt.seqnum < cliSeqNum) { //server receives already acked packet and sends dupack
                        buildPkt(&ackpkt, recvpkt.acknum, (recvpkt.seqnum + recvpkt.length) % MAX_SEQN, 0, 0, 0, 1, 0, NULL);
                        printSend(&ackpkt, 0);
                        sendto(sockfd, &ackpkt, PKT_SIZE, 0, (struct sockaddr*) &cliaddr, cliaddrlen);
                    } 
                    else if (recvpkt.seqnum > cliSeqNum) { //server receives out of order packet and sends dupack for current packet
                        buildPkt(&ackpkt, recvpkt.acknum, cliSeqNum, 0, 0, 0, 1, 0, NULL);
                        printSend(&ackpkt, 0);
                        sendto(sockfd, &ackpkt, PKT_SIZE, 0, (struct sockaddr*) &cliaddr, cliaddrlen);
                    } 
                    else { //server receives correct packet, sends ack, and updates variables accordingly
                        buildPkt(&ackpkt, recvpkt.acknum, (recvpkt.seqnum + recvpkt.length) % MAX_SEQN, 0, 0, 1, 0, 0, NULL);
                        printSend(&ackpkt, 0);
                        sendto(sockfd, &ackpkt, PKT_SIZE, 0, (struct sockaddr*) &cliaddr, cliaddrlen);
