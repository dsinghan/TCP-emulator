...
int main (int argc, char *argv[])
{
    ...
    //       handling data loss.
    //       Only for demo purpositione. DO NOT USE IT in your final submission

    int counter = 1; //num of packets sent out currently
    int position = 1; //position of most recently sent out packet
    int oldest_position = 0; //position of least recent unacked packet
    seqNum = (seqNum + m) % MAX_SEQN;
    m = fread(buf, 1, PAYLOAD_SIZE, fp);

    while (counter < 10 && m > 0) { //send first 10 packets
        buildPkt(&pkts[position], seqNum, (synackpkt.seqnum + 1) % MAX_SEQN, 0, 0, 0, 0, m, buf);
        printSend(&pkts[position], 0);
        sendto(sockfd, &pkts[position], PKT_SIZE, 0, (struct sockaddr*) &servaddr, servaddrlen);
        counter++;
    ...
    while (counter != 0) { //send the remaining packets
        n = recvfrom(sockfd, &ackpkt, PKT_SIZE, 0, (struct sockaddr *) &servaddr, (socklen_t *) &servaddrlen);
        if (n > 0 && ackpkt.acknum == (pkts[oldest_position].seqnum + pkts[oldest_position].length) % MAX_SEQN) { //ack for oldest unacked packet is received
            timer = setTimer();
            oldest_position = (oldest_position + 1) % WND_SIZE;
            printRecv(&ackpkt);
            counter--;
        }
        if (m > 0 && counter < 10) { //more packets to send
            buildPkt(&pkts[position], seqNum, (ackpkt.seqnum) % MAX_SEQN, 0, 0, 0, 0, m, buf);
            printSend(&pkts[position], 0);
            sendto(sockfd, &pkts[position], PKT_SIZE, 0, (struct sockaddr*) &servaddr, servaddrlen);
            counter++;
            position = (position + 1) % 10;
        ...
        if (isTimeout(timer)) { //timer for oldest unacked packet times out
            printTimeout(&pkts[oldest_position]);
            for (int i = oldest_position; i < oldest_position + counter; i++) {
